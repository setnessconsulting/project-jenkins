...
- Canceling a build during Standard CI stopped the command, produced a neutral/not-run Candidate result, and removed a marker injected after checkout. The primary check was rendered as failure rather than canceled by the automatic publisher; this is fail-closed but should be considered in reviewer-output validation.
...
- Plain `.md` files are excluded from Candidate path classification; `.mdx` remains eligible because it is validated and built. A mergeable docs-only PR now publishes a neutral/not-applicable Candidate result before Standard CI, and a marker inserted after checkout was removed with the completed workspace.
- GitHub's synthetic PR merge revision has the PR head as first parent and target/base as second parent. Candidate classification now compares against the base parent; the earlier first-parent comparison included unrelated target history and overclassified changes.
...
- The lifecycle publisher replaces the primary check's custom summary on completion. The check's text now contains named suite-level stages and a final result line; failure output includes the failing stage and log excerpt. A successful check may still have an empty `output.summary`, so verify this reviewer experience before cutover rather than assuming the custom summary survived.
- A rejected PR can fail before checkout, when the Pipeline Checks step lacks a resolved revision and cannot publish the intended head-SHA result. That PR receives no passing Jenkins result and remains blocked by a missing required check; do not describe this as a visible Jenkins failure check. Resolve the UX before treating Jenkins as the merge gate. Direct fork PRs remain undiscovered and therefore blocked when Jenkins becomes required.
...
- Candidate changed-path classification compares the synthetic merge revision with its target/base parent (`HEAD^2`), not the PR-head parent.
