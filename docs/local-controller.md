...
- A docs-only not-applicable case is still unproven. A prior conflicting PR could not produce a synthetic merge revision, and a later documentation-heavy PR changed configured Candidate paths and therefore was not a not-applicable case. Use a mergeable docs-only PR; do not change PR checkout from merge revision to head merely to bypass mergeability.
...
- Jenkins’ configured root URL is intentionally loopback-only, so check-detail links work only in a browser on this same machine. Do not expose the controller to make those links work for remote users. A successful primary check currently has an empty `summary` field but its `text` lists the completed stages and durations; the separate Candidate check has a human-readable outcome/path summary. A pipeline-published summary was overwritten by the Checks plugin's automatic completion update, so that ineffective code was removed. Verify the rendered combined output is sufficient for reviewers before cutover; if not, solve the context design without creating an ambiguous duplicate required check.
...
- The primary check name is `jenkins-pr-gate`. Cloudflare Candidate path rules are supplied from local config and injected into the trusted inline pipeline; non-relevant PRs receive an explicit neutral `cloudflare-candidate` result saying “not applicable.” The primary check conclusion governs the merge; the separate Candidate summary never substitutes for a passing primary gate.
