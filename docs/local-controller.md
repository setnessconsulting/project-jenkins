...
- Jenkins’ configured root URL is intentionally loopback-only, so check-detail links work only in a browser on this same machine. Do not expose the controller to make those links work for remote users. Earlier runs showed an empty primary-check summary because the Checks plugin's automatic completion update overwrote the Pipeline summary. The follow-up removes that automatic status-check publisher and explicitly publishes both check summaries from the trusted Pipeline. Confirm the rendered primary and Candidate summaries on live PRs before calling shadow usable or considering cutover.
...
- The primary check name is `jenkins-pr-gate`. Cloudflare Candidate path rules are supplied from local config and injected into the trusted inline pipeline; non-relevant PRs receive an explicit neutral `cloudflare-candidate` result saying “not applicable.” Both checks are explicitly published by the trusted Pipeline with reviewer-readable summaries. The primary check conclusion governs the merge; the separate Candidate summary never substitutes for a passing primary gate.
