...
- Plain `.md` files are now excluded from Candidate path classification; `.mdx` remains eligible because it is validated and built. A live docs-only not-applicable check is still pending; marker cleanup passed on a canceled build.
